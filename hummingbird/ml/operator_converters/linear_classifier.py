...
class LogisticRegression(torch.nn.Module):
    def __init__(self, coefficients, intercepts, classes, multi_class, device, is_lin_classif=False):
        ...
        self.is_lin_classif = is_lin_classif
...
def convert_sklearn_linear_model(operator, device, extra_config):
    ...
    if not all([type(x) in [int, np.int32, np.int64] for x in classes]):
        raise RuntimeError("skl2pytorch supports only integer labels for class labels.")
    ...
    return LogisticRegression(coefficients, intercepts, classes, multi_class, device, is_lin_classif)
